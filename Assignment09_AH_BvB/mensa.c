...
String get_list(Node* list, int position) {
    require("valid position of list", list != NULL);
    if (position == 0) {
        return list->value;
    } else {
        return get_list(list->next, position - 1);
    }
}
...
void free_list(Node* list) {
    ...
    // "own" solution:
    /* if (list != NULL) {
        // remove first element of list (length >= 2)
...
Node* append_list(Node* list, String value) {
    Node* add_node = xcalloc(1, sizeof(Node));
    add_node->value = s_copy(value); // s_copy performs dynamic allocation
    add_node->next = NULL;
...
void print_situation(void) {
    ...
    printf("Reputation der Mensa: %2d\n", reputation);
    printf("> ");
}
...
void finish(void) {
    free_list(food);
    free_list(students);
    printf("Fertig fuer heute. Die Mensa schliesst.\nFinale Reputation der Mensa: %2d\n", reputation);
    exit(0);
}

const int NUMBER_OF_COMPLETED_FOODS = 5;
const int STUDENTS_AT_START = 3;
...
void run_mensa(void) {
    ...
    while ((i = i_input()) >= -1) {
        if (i == -1) {
            ...
            reputation -= 2;
        } else if (i <= NUMBER_OF_COMPLETED_FOODS) {
            String demanded_food = get_list(students, 0);
            String chosen_food = get_list(food, i);
            
            if (s_equals(chosen_food, demanded_food)) {
                ...
                reputation--;
                printf("%s moechte ich nicht! Ich moechte %s!", chosen_food, demanded_food);
                printf(" Dann hole ich mir halt einen Doener!\n");
            }
            
            students = remove_list(students, 0);
            
        } else {
            printf("Konzentrierter Arbeiten: Es gibt nur %d fertige Essen und nicht %d.\n", NUMBER_OF_COMPLETED_FOODS, i);
            printf("> ");
